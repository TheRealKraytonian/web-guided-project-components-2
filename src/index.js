...
// http vs. tcp/ip
// lambda-times-api.herokuapp.com/ -- human-readable form of the IP address
...
//   .then((futureData) => console.log("2. here is our future data!", futureData))
//   .catch((drama) => console.log(drama));

// console.log("3. we requested data with axios!");

// Before writting code to use the data returned from the API,
...
fetch("https://lambda-times-api.herokuapp.com/friends")
  ...
  .then((boduOfResponse) => console.log(boduOfResponse))
  .catch();
...
//    an they can rin our index.js untill they have downloaded it
...
function getDogs(breed, numDogs) {
  axios
    .get(`https://dog.ceo/api/${breed}/mastiff/images/random/${numDogs}`)
    .then((res) => {
      res.data.message.forEach((imageURL) => {
        const dogCard = dogCardMaker({ imageURL, breed: `${breed}` });
        entryPoint.appendChild(dogCard);
      });
    })
    .catch((err) => {
